...
// Let the switch be in PIN0 of PORTA 

// Let the led on PIN1 of PORTA 

// Let the temperature sensor to be in PIN2 of PORTA
...
void adc0_init(void)
{
	...
	//ADC0_SSPRI_R = (ADC0_SSPRI_R&0xFFFF0FFF) | (0x00001000); // Set prioirty of ss3 to 1
	//ADC0_SPC_R = 0 ; // Set sample phase control to lag of 0 degree
}
...
void TaskOfLedAndSensor(void)
{
	Portf_init();
	timer0_module_a_init();
	adc0_init();
	uart0_init();
}
